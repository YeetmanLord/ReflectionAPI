...
import com.github.yeetmanlord.reflection_api.players.player_connection.NMSPlayerConnectionReflection;
...
public class NMSPlayerReflection extends NMSEntityReflection {
	...
	public NMSPlayerReflection(NMSServerReflection server, NMSWorldServerReflection world, GameProfile profile,
			NMSPlayerInteractManagerReflection interactManager) throws Exception {
		super(ReflectionApi.getNMSClass("EntityPlayer")
				.getConstructor(server.getNmsServer().getClass().getSuperclass(), world.getNmsWorldServer().getClass(),
						profile.getClass(), interactManager.getNmsManager().getClass())
				.newInstance(server.getNmsServer(), world.getNmsWorldServer(), profile,
						interactManager.getNmsManager()));
